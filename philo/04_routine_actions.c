...
/*   Updated: 2025/02/27 14:39:15 by icunha-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pre_dinner_check(t_philo *philo)
{
	...
	{
		if (time_passed >= time_to_die && get_bool(philo->data, &philo->ph_mtx, &philo->ph_eating)) // ver se é preciso check ph_eating
		{
		...
		if(get_bool(philo->data, &philo->ph_mtx, &philo->data->all_ph_full) && !end_dinner(philo->data, NULL, MEAL_END))
		{
			set_bool_var(philo->data, &philo->data->data_mtx, &philo->data->end_dinner, true);
			return (1);
		}
...
//	set_bool_var(philo->data, &philo->ph_mtx, &philo->ph_eating, true); //check if needed
